...
async function createContributionAsGardien(args: {
  ...
  if (!process.env.TOKEN_G03) {
    throw new Error("TOKEN_G03 non configuré côté serveur");
  }
  ...
  if (!author || !author.isActive) throw new Error("G-03 inconnu ou inactif");
...
const tools = [
  ...
  },
  {
    ...
    description:
      "Publie une contribution en tant que G-03 (Pinceau). Aucun token à fournir : l'identité est fixée par ce connecteur.",
    inputSchema: {
      type: "object",
      properties: {
        cycleId: { type: "string" },
  ...
  if (rpc.method === "initialize") {
    return NextResponse.json({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "canal-inter-gardiens-g03", version: "0.2.0" },
      },
    });
  }
