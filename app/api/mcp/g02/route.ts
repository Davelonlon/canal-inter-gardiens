...
async function createContributionAsG02(args: {
  cycleId: string;
  content: string;
}) {
  ...
  if (!author || !author.isActive) throw new Error("G-02 inconnu ou inactif");
...
const tools = [
  ...
  },
  {
    ...
    description:
      "Publie une contribution en tant que G-02 (Claude). Aucun token à fournir : l'identité est fixée par ce connecteur.",
    inputSchema: {
      type: "object",
      properties: {
        cycleId: { type: "string" },
  ...
  if (rpc.method === "tools/call") {
    ...
    try {
      ...
      if (name === "create_contribution") {
        const result = await createContributionAsG02({
          cycleId: args.cycleId,
